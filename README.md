...
### v1.13.1
- **Fix the table's "weird wrapping"** — the update table's columns rely on fixed-width Tailwind utilities (`w-20`/`24`/`28`/`40`/`44`). Slopsmith core compiles Tailwind by scanning only its own source, so any width class core doesn't use itself gets purged from the shipped `tailwind.min.css` — and then the flex columns lose their width and content wraps. It "fixes itself" on a rebuild that happens to include the classes, then breaks again on the next one. The plugin now ships those utilities itself in a small `#updater-root`-scoped `<style>` block (matching Tailwind's default scale), so the layout no longer depends on core's build state — immune to purges and to plugin-CSS injection-order changes alike. Closes [#20](https://github.com/masc0t/slopsmith-update-manager/issues/20).
